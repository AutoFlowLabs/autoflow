...
test.describe("lobster", () => {
  test("Lobster Recent Stories Count", async ({ page }) => {
    ...
    const numStories = (await autoflow("How many stories are listed?", { page, test })) as string;
...
test.describe("saucedemo", () => {
  test("Shopping and Checkout", async ({ page }) => {
    ...
    await autoflow(`Fill the 'username' with a standard_user'`, { page, test }, "action");
    ...
    await autoflow(`click 'add to cart' button of item with name 'Sauce Labs Backpack'`, { page, test },"action");
    ...
    await autoflow(`Click the 'shopping cart icon'`, { page, test });
    ...
    await autoflow(`fill the 'last name' with ${lastNameForSauce}`, { page, test });
...
test.describe("herokuapp", () => {
  test("add/remove elements", async ({ page }) => {
    ...
    await autoflow(`Click on the 'add element' button`, { page, test }, "action");
  ...
  test("get the text in iframe", async ({ page }) => {
    ...
    const text = await autoflow(`get the text in the iframe`, { page, test }, "query");
    console.log(text);
  });
